const { CommandThrottling } = require("../src/services/commandThrottlingService");
const commandThrottlingService = require("../src/services/commandThrottlingService").CommandThrottling;
 ...
 * @return  {void}
 */
const injectTestThrottling = (value: any = testCommandCooldown) => {
...
describe("Channel Throttling Service", () => {
  ...
  describe("Throttling Setup", () => {
    it("should fetch the existing command throttling key from the .env", () => {
      ...
      expect(fetchedThrottles.includes(testCommandThrottle)).toBeTruthy();
    });
    ...
    it("should not find a test command in fetched throttles", () => {
      ...
      expect(fetchedThrottles.includes(testCommandThrottle)).toBeFalsy();
    });
  ...
  describe("Throttling Application", () => {
    ...
    it("should reject a command on cooldown", () => {
      ...
      expect(throttlingService.canBeExecuted()).toBeFalsy();
    });
    ...
    */
    for (const execution of Object.values(executions)) {
      const testCooldown = <number>execution[0];
      const expectedResult = execution[1];
      ...
      it(`(async test) should ${execution} the command execution after ${testCooldown} milliseconds`, async () => {
        ...
        expect(cooldownState).toBe(expectedResult);
      });
    };
  });
});
